...
* NEW! Fast switch between time trackers. Time tracker spent time is automatically stored as time entry.
* NEW! Updated time trackers list (index) page. Shows most recent tasks updated / worked on by current user instead of paused time trackers.
...
The newest version is always at {GitHub}[http://github.com/egisz/redmine_time_tracker].
...
Not updated, see Git section

=== Git

1. Open a shell to your Redmine's <tt>plugins/redmine_time_tracker</tt> folder
2. Update your git copy with <tt>git pull</tt>
3. Update the database using the migrations. <tt>rake redmine:plugins:migrate RAILS_ENV=production</tt>

class TimeTrackersController < ApplicationController
  ...
  def index
    @time_trackers = {}
    if User.current.nil?
      @time_trackers[:other] = TimeTracker.find(:all)
      @time_trackers[:user] = []
    else
      @time_trackers[:other] = TimeTracker.find(:all, :conditions => ['user_id != ?', User.current.id])
      @time_trackers[:user] = TimeTracker.find(:all, :conditions => { :user_id => User.current.id })
    end
  end
